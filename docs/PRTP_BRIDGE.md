...
[`DIGIME_RECONFIG.md`](DIGIME_RECONFIG.md). The script queries and backs up the
original settings before deployment by default, writes a manifest and the
planned RCI changes under `digime/backups/<timestamp>/`, supports restoring that
backup, and does not modify the modules unless `--yes` is supplied. Preserve the
backup directory until the installation has been validated.
